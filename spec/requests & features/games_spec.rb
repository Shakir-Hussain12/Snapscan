...
RSpec.describe 'Games', type: :request do
  before(:each) do
    ...
    @game = @user.games.create(name: 'test_game', amount: 10)
  ...
  describe 'POST categories/:id/games' do
    it 'returns http success' do
      post category_games_path(@category.id), params: { game: { name: 'test_game', amount: 10 } }
    ...
    it 'does not create a game on invalid params' do
      post category_games_path(@category.id), params: { game: { name: '', amount: 10 } }
      expect(@user.games.count).to eq(1)
    end

    it 'page to have a game on valid params' do
      post category_games_path(@category.id), params: { game: { name: 'test_game1', amount: 10 } }
      get category_games_path(@category.id)
      expect(response.body).to include('test_game')
    end

    it 'page to have an amount on valid params' do
      post category_games_path(@category.id), params: { game: { name: 'test_game', amount: 20 } }
      get category_games_path(@category.id)
